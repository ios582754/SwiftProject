...
//let ApiLoadingProvider = MoyaProvider<Homeap>(requestClosure: timeoutClosure, plugins: [LoadingPlugin])
...
extension MoyaProvider {
    ...
    open func request<T: HandyJSON>(_ target: Target,
        ...
        return request(target, completion: { (result) in
            print(result.value ?? "")
            if target.self is LBUHomeAPI {
                guard let completion = completion else { return }
                guard let returnData = try? result.value?.mapModel(QJBananer.self) else {
                               completion(nil)
                               return
                           }
