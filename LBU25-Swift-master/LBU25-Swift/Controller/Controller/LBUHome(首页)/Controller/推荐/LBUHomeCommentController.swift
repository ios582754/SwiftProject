...
class LBUHomeCommentController: LBUBaseController, LLCycleScrollViewDelegate {
    ...
    private var bananerItems = [QJData]()
    private var textItems = [LBUTextItemModel]()
    private var comicLists = [LBUComicListModel]()
    open var myBlock: ((_ offsetX:CGFloat, _ index:Int,_ colors:[String]) -> ())?
    ...
    func cycleScrollView(_ cycleScrollView: LLCycleScrollView, scrollTo index: NSInteger) {
        let colors = self.bananerItems.filter { $0.color != nil }.map { $0.color! }
        
        myBlock?(0 ,index,colors)
    }
    // 懒加载
    ...
    private func setupLoadData(_ changeSex: Bool) {
        ...
        HomeLoadingProvider.request(LBUHomeAPI.bananerList(type: 1),model: QJBananer.self) { [weak self] (returnData) in
            print(returnData?.data)
             self?.bananerItems = returnData?.data ?? []
             self?.bannerView.imagePaths = self?.bananerItems.filter { $0.photoPath != nil }.map { $0.photoPath! } ?? []
